...
<?php $pageTitle = isset($dest) ? 'Edit Destination' : 'Create Destination'; ob_start(); ?>
<div class="max-w-4xl mx-auto px-4 py-10">
  <div class="flex items-center justify-between mb-6">
    <h1 class="text-2xl font-semibold"><?php echo htmlspecialchars($pageTitle); ?></h1>
    <a class="btn btn-secondary" href="<?php echo base_url('/travel/destinations'); ?>">Back</a>
  </div>
    ...
    <form method="post" enctype="multipart/form-data">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <input type="text" name="title" class="input w-full" required value="<?php echo htmlspecialchars($dest['title'] ?? ''); ?>">
        </div>
        <div>
          <label class="block text-sm text-gray-400">Destination *</label>
          <input type="text" name="destination" class="input w-full" required value="<?php echo htmlspecialchars($dest['destination'] ?? ''); ?>">
        </div>
        <div>
          <label class="block text-sm text-gray-400">Duration (days)</label>
          <input type="number" name="duration_days" min="1" value="<?php echo htmlspecialchars($dest['duration_days'] ?? '1'); ?>" class="input w-full">
        </div>
        <div>
          <label class="block text-sm text-gray-400">Price *</label>
          <input type="number" step="0.01" name="price" class="input w-full" required value="<?php echo htmlspecialchars($dest['price'] ?? ''); ?>">
        </div>
        <div>
          <label class="block text-sm text-gray-400">Currency</label>
          <input type="text" name="currency" value="<?php echo htmlspecialchars($dest['currency'] ?? 'KES'); ?>" class="input w-full">
        </div>
        <div>
          <label class="block text-sm text-gray-400">Departure Location *</label>
          <input type="text" name="departure_location" class="input w-full" required value="<?php echo htmlspecialchars($dest['departure_location'] ?? ''); ?>">
        </div>
        <div>
          <label class="block text-sm text-gray-400">Departure Date *</label>
          <input type="date" name="departure_date" class="input w-full" required value="<?php echo htmlspecialchars($dest['departure_date'] ?? ''); ?>">
        </div>
        <div>
          <label class="block text-sm text-gray-400">Return Date</label>
          <input type="date" name="return_date" class="input w-full" value="<?php echo htmlspecialchars($dest['return_date'] ?? ''); ?>">
        </div>
        <div>
          <label class="block text-sm text-gray-400">Booking Deadline</label>
          <input type="date" name="booking_deadline" class="input w-full" value="<?php echo htmlspecialchars($dest['booking_deadline'] ?? ''); ?>">
        </div>
        <div>
          <label class="block text-sm text-gray-400">Max Participants</label>
          <input type="number" name="max_participants" value="50" class="input w-full">
        </div>
        <div>
          <label class="block text-sm text-gray-400">Min Participants</label>
          <input type="number" name="min_participants" value="1" class="input w-full">
        </div>
        ...
        <div class="md:col-span-2">
          ...
          <input type="file" name="gallery[]" class="input w-full" multiple>
        </div>
        <div class="md:col-span-2">
          <label class="block text-sm text-gray-400">Description</label>
          <textarea name="description" rows="3" class="input w-full"><?php echo htmlspecialchars($dest['description'] ?? ''); ?></textarea>
        </div>
        <div>
          <label class="block text-sm text-gray-400">Includes (one per line)</label>
          <textarea name="includes" rows="3" class="input w-full"><?php echo isset($dest['includes']) ? htmlspecialchars(implode("\n", json_decode($dest['includes'] ?: '[]', true) ?: [])) : ''; ?></textarea>
        </div>
        <div>
          <label class="block text-sm text-gray-400">Excludes (one per line)</label>
          <textarea name="excludes" rows="3" class="input w-full"><?php echo isset($dest['excludes']) ? htmlspecialchars(implode("\n", json_decode($dest['excludes'] ?: '[]', true) ?: [])) : ''; ?></textarea>
        </div>
        <div class="md:col-span-2">
          <label class="block text-sm text-gray-400">Requirements/Notes (one per line)</label>
          <textarea name="requirements" rows="3" class="input w-full"><?php echo isset($dest['requirements']) ? htmlspecialchars(implode("\n", json_decode($dest['requirements'] ?: '[]', true) ?: [])) : ''; ?></textarea>
        </div>
        <div class="md:col-span-2">
          <label class="block text-sm text-gray-400">Itinerary (one per line)</label>
          <textarea name="itinerary" rows="4" class="input w-full"><?php echo isset($dest['itinerary']) ? htmlspecialchars(implode("\n", json_decode($dest['itinerary'] ?: '[]', true) ?: [])) : ''; ?></textarea>
        </div>
