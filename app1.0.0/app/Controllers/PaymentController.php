...
class PaymentController
{
	...
	public function flutterwave(): void
	{
        ...
        $txRefParam = $_GET['tx_ref'] ?? '';
        if ($status !== '' && ($txId !== '' || $txRefParam !== '')) {
            $base = 'https://api.flutterwave.com'; // same for sandbox/live
            // Prefer verify by transaction_id; fallback to verify by reference when tx_id missing
            if ($txId !== '') {
                $verifyUrl = $base . '/v3/transactions/' . urlencode($txId) . '/verify';
            } else {
                $verifyUrl = $base . '/v3/transactions/verify_by_reference?tx_ref=' . urlencode($txRefParam);
            }
            ...
            $txRef = $json['data']['tx_ref'] ?? $txRefParam;
