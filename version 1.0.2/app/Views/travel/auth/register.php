...
<div class="min-h-screen flex items-center justify-center py-8 px-4 sm:px-6 lg:px-8">
    <div class="max-w-2xl w-full space-y-6">
        ...
        <form class="mt-6 space-y-4" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input id="company_name" name="company_name" type="text" required 
                           class="mt-1 appearance-none relative block w-full px-3 py-2 border border-gray-600 placeholder-gray-400 text-gray-900 bg-white rounded-md focus:outline-none focus:ring-red-500 focus:border-red-500 sm:text-sm" 
                           placeholder="Your travel company name">
                </div>
                <div>
                    <label for="contact_person" class="block text-sm font-medium text-gray-200">Contact Person *</label>
                ...
                <div>
                    <label for="website" class="block text-sm font-medium text-gray-200">Website</label>
                ...
                <div>
                    ...
                    <select id="country" name="country" class="mt-1 block w-full px-3 py-2 border border-gray-600 bg-white text-gray-900 rounded-md focus:outline-none focus:ring-red-500 focus:border-red-500 sm:text-sm"></select>
                </div>
                <div>
                    <label for="city" class="block text-sm font-medium text-gray-200">City</label>
                    <select id="city" name="city" class="mt-1 block w-full px-3 py-2 border border-gray-600 bg-white text-gray-900 rounded-md focus:outline-none focus:ring-red-500 focus:border-red-500 sm:text-sm"></select>
                </div>
                <div>
            ...
            <div>
                <label for="description" class="block text-sm font-medium text-gray-200">Company Description</label>
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  function onCountryChange(){
    ...
    if(phone){
      ...
      const countryCodeDigits = c.code.replace(/\D+/g,''); // e.g., "254" from "+254"
      if(digits.startsWith(countryCodeDigits)){
        digits = digits.substring(countryCodeDigits.length);
      }
      // Remove leading zeros
      digits = digits.replace(/^0+/, '');
      // Set formatted phone with country code
      phone.value = c.code + ' ' + digits;
    }
    // Populate cities
    if(city){
