				...
				<form method="post" action="<?php echo base_url('/orders'); ?>" class="space-y-4">
                    ...
                    <div class="mt-6 grid md:grid-cols-3 gap-3">
                        <div class="md:col-span-1">
                            ...
                            <input type="text" name="name" placeholder="Your name" class="w-full px-3 py-2 border border-gray-600 bg-gray-800 text-white placeholder-gray-400 rounded-md focus:outline-none focus:ring-red-500 focus:border-red-500" required>
                        </div>
                        <div class="md:col-span-1">
                            <label class="block text-sm font-medium text-white mb-2">Email (optional)</label>
                            <input type="email" name="email" placeholder="you@example.com" class="w-full px-3 py-2 border border-gray-600 bg-gray-800 text-white placeholder-gray-400 rounded-md focus:outline-none focus:ring-red-500 focus:border-red-500">
                        </div>
                        <div class="md:col-span-1">
                            <label class="block text-sm font-medium text-white mb-2">Mobile Number (required)</label>
                            <input type="tel" name="msisdn" id="phoneInput" placeholder="254712345678" class="w-full px-3 py-2 border border-gray-600 bg-gray-800 text-white placeholder-gray-400 rounded-md focus:outline-none focus:ring-red-500 focus:border-red-500" style="color: white !important; background-color: #1f2937 !important;" autocomplete="tel" required>
                            <div class="text-xs text-gray-400 mt-1">Used for M-Pesa payment and ticket SMS</div>
                        </div>
                    </div>

                    <!-- M-Pesa Phone Number (shown when M-Pesa is selected) -->
                    <div id="mpesa-phone" class="mt-4">
                        <label class="block text-sm font-medium text-white mb-2">Confirm M-Pesa Phone Number</label>
                        <input type="tel" name="msisdn_confirm" id="phoneConfirm" placeholder="254712345678" class="w-full px-3 py-2 border border-gray-600 bg-gray-800 text-white placeholder-gray-400 rounded-md focus:outline-none focus:ring-red-500 focus:border-red-500" style="color: white !important; background-color: #1f2937 !important;" value="<?php echo htmlspecialchars($userPhone); ?>" autocomplete="tel">
                        <div class="text-sm text-gray-400 mt-1">We’ll send STK push to this number</div>
                        <div id="phoneMismatch" class="text-sm text-red-400 mt-2 hidden">Phone numbers do not match.</div>
					</div>
					...
					<!-- Submit Button -->
                    <button id="submitBtn" type="submit" class="btn btn-primary w-full text-lg py-3 mt-4" <?php echo (!empty($settings['mpesa']) || !empty($settings['paypal']) || !empty($settings['flutterwave'])) ? '' : 'disabled title="Enable gateways in Admin > Settings"'; ?> onclick="formatPhoneBeforeSubmit()">
						Proceed to Payment
					</button>
				</form>
			</div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function normalizeTo254(value){
    let v = (value || '').replace(/\D/g, '');
    if (v === '') return '';
    if (v.startsWith('254')) return v;
    if (v.startsWith('0')) return '254' + v.substring(1);
    if (v.length >= 9) return '254' + v;
    return v;
  }
  function attachFormatters(el){
    if (!el) return;
    el.addEventListener('input', function(e) {
      let value = e.target.value.replace(/\D/g, ''); // Remove non-digits
      
      // Auto-format Kenyan phone numbers for M-Pesa
      if (value.length > 0) { value = normalizeTo254(value); }
      
      e.target.value = value;
    });
    el.addEventListener('blur', function(e){ e.target.value = normalizeTo254(e.target.value); });
    if (el.value) { el.value = normalizeTo254(el.value); }
  }
  attachFormatters(phoneInput);
  attachFormatters(phoneConfirm);

  // Live mismatch validation
  function validatePhonesMatch(){
    const a = normalizeTo254(phoneInput ? phoneInput.value : '');
    const b = normalizeTo254(phoneConfirm ? phoneConfirm.value : '');
    const warn = document.getElementById('phoneMismatch');
    const submit = document.getElementById('submitBtn');
    const shouldWarn = !!a && !!b && a !== b;
    if (warn) { warn.classList.toggle('hidden', !shouldWarn); }
    if (submit) { submit.disabled = shouldWarn; }
  }
  if (phoneInput) phoneInput.addEventListener('input', validatePhonesMatch);
  if (phoneConfirm) phoneConfirm.addEventListener('input', validatePhonesMatch);
  validatePhonesMatch();
});
...
function formatPhoneBeforeSubmit() {
  const phoneInput = document.getElementById('phoneInput');
  const phoneConfirm = document.getElementById('phoneConfirm');
  if (phoneInput) { phoneInput.value = normalizeTo254(phoneInput.value); }
  if (phoneConfirm) { phoneConfirm.value = normalizeTo254(phoneConfirm.value); }
  // Ensure the msisdn actually submitted is the confirmed one
  const form = document.querySelector('form[action*="/orders"]');
  if (form) {
    let existing = form.querySelector('input[name="msisdn"]');
    if (!existing) {
      existing = document.createElement('input');
      existing.type = 'hidden';
      existing.name = 'msisdn';
      form.appendChild(existing);
    }
    existing.value = phoneConfirm && phoneConfirm.value ? phoneConfirm.value : (phoneInput ? phoneInput.value : '');
  }
}
