...
class UserController
{
	...
	public function accountUpdate(): void
	{
		...
		// Detect first-time email add (previously empty)
		$prevStmt = db()->prepare('SELECT email FROM users WHERE id = ?');
		$prevStmt->execute([$_SESSION['user_id']]);
		$prev = $prevStmt->fetch();
		$wasEmpty = !($prev && trim((string)($prev['email'] ?? '')) !== '');
		$stmt = db()->prepare('UPDATE users SET phone = ?, email = ? WHERE id = ?');
		try {
			$stmt->execute([$phone, $email, $_SESSION['user_id']]);
			// Award points if this is the first time the user adds an email
			try {
				if ($wasEmpty && $email !== '') {
					$award = (int)\App\Models\Setting::get('loyalty.points.add_email', '0');
					if ($award > 0) {
						$db = db();
						$db->prepare('INSERT INTO user_points (user_id, points, reason, reference_type, reference_id, created_at) VALUES (?, ?, ?, NULL, NULL, NOW())')
							->execute([$_SESSION['user_id'], $award, 'Add email']);
					}
				}
