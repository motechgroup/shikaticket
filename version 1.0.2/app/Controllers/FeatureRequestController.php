...
class FeatureRequestController
{
    ...
    {
        require_admin();
        // Only allow POST approvals; if accessed via GET, redirect with a message instead of 500
        if (($_SERVER['REQUEST_METHOD'] ?? 'GET') !== 'POST') {
            flash_set('error', 'Invalid request method. Please use the Approve button from the admin page.');
    ...
    public function rejectRequest(): void
    {
        require_admin();
        if (($_SERVER['REQUEST_METHOD'] ?? 'GET') !== 'POST') {
            flash_set('error', 'Invalid request method. Please use the Reject button from the admin page.');
            redirect(base_url('/admin/featured-content'));
            return;
        }
        verify_csrf();
